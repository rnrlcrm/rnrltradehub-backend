...
This module provides API endpoints specific to the vendor portal where
vendor users can view and manage their contracts, invoices, payments, and profile.
"""
...
vendor_router = APIRouter(prefix="/api/vendor", tags=["Vendor Portal"])
...
class BusinessPartnerProfile(BaseModel):
    """Business partner profile for vendor."""
    id: str
    bp_code: str
    legal_name: str
    organization: str
...
# Helper function to ensure user is VENDOR
def require_vendor_user(current_user: models.User = Depends(get_current_user)):
    """Ensure the current user is a VENDOR user."""
    if current_user.user_type != 'VENDOR':
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Access denied. This endpoint is for vendor users only."
        )
    return current_user


# Contract Endpoints
@vendor_router.get("/my-contracts", response_model=List[ContractSummary])
def list_my_contracts(
    skip: int = Query(0, ge=0),
    limit: int = Query(100, ge=1, le=100),
    status: Optional[str] = None,
    current_user: models.User = Depends(require_vendor_user),
    db: Session = Depends(get_db)
):
    """
    Get all contracts where the user is the vendor (seller).
...
@vendor_router.get("/my-contracts/{contract_id}")
def get_contract_details(
    ...
    contract = db.query(models.SalesContract).filter(
        ...
        models.SalesContract.vendor_id == bp_id
    ).first()
...
# Invoice Endpoints
@vendor_router.get("/my-invoices", response_model=List[InvoiceSummary])
...
# Payment Endpoints
@vendor_router.get("/my-payments", response_model=List[PaymentSummary])
def list_my_payments(
    ...
    """
    Get all payments received by the user.
...
# Profile Endpoints
@vendor_router.get("/my-profile", response_model=BusinessPartnerProfile)
def get_my_profile(
    current_user: models.User = Depends(require_vendor_user),
    db: Session = Depends(get_db)
):
...
@vendor_router.post("/sub-users", response_model=SubUserResponse, status_code=status.HTTP_201_CREATED)
def create_sub_user(
...
@vendor_router.get("/sub-users", response_model=List[SubUserResponse])
def list_sub_users(
    current_user: models.User = Depends(require_vendor_user),
    db: Session = Depends(get_db)
):
...
@vendor_router.delete("/sub-users/{sub_user_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_sub_user(
    sub_user_id: int,
    current_user: models.User = Depends(require_vendor_user),
    db: Session = Depends(get_db)
